...
Это русскоязычный max бот - бухгалтер репетитора. Позволяет взаимодействовать с балансами учеников хранимыми в виде .json файлов.
Также учитывает индивидуальные и групповые уроки из Google календаря репетитора и при пересчете вычитает из балансов учеников нужные суммы.
...
* В папке проекта сделать config.json в следующем формате:
  ...
  {
    "token": "ВАШ MAX ТОКЕН",
    "users": ["MAX ID ТЕХ КТО БУДЕТ ИМЕТЬ ДОСТУП"],
  ...
  ```
* Создать .json файлы (кроме logs и google_token, они создаются автоматически при первом запуске) по примеру из data.example по путям указанным в config<br>
...
* При первом запуске понадобится залогиниться в Гугле в браузере
