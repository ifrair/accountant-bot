...
# makes report about this and prev month, prev 30 days, pre-prev 30 days,
# prev 4 weeks, all the time
...
def make_report():
    ...
    for sum_list in sums.values():
        for period_sum in sum_list:
            int(period_sum.money / (period_sum.hours if period_sum.hours > 0 else 1))
            rep_text += (
                f"{period_sum.period}:"
                + " " * int((25 - len(period_sum.period)) * 2)
                + f" {period_sum.money} руб, за {round(period_sum.hours, 1)} часов,"
                + " ~ {avg} руб/ч\n\n"
            )
