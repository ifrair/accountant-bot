...
def connect_to_calendar():
    ...
    creds = None
    if Path(config_json["google_token"]).exists():
        creds = Credentials.from_authorized_user_file(config_json["google_token"], url)
    if not creds or not creds.valid:
        flow = InstalledAppFlow.from_client_secrets_file(config_json["credentials"], url)
