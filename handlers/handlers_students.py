...
async def add_new_price(event: MessageCreated, context: MemoryContext):
    ...
    await event.message.answer(f"Добавить ученика: {message_data['name']} с балансом: {message_data['price']}?",
...
async def delete_name(event: MessageCallback, context: MemoryContext):
    ...
    await event.message.answer(f'Удалить {response}?',
                         attachments=[keyboards.delete_student])
