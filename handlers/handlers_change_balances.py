...
@router.message_callback(None, (F.callback.payload == 'add_balance') | (F.callback.payload == 'subtract_balance'))
@check_access
async def change_money(event: MessageCallback, context: MemoryContext):
    money_counts = take_from_json("money_count")
    await event.message.delete()
    if money_counts == {}:
        await event.message.answer('У вас нет учеников.\nЧто для Вас сделать?',
...
async def get_name(event: MessageCallback, context: MemoryContext):
    ...
    await event.message.answer(
        ('Пополняем' if message_data['is_add'] else 'Вычитаем') + f' {response}...\nТеперь напиши сколько ' + (
...
async def approving_changes(event: MessageCallback, context: MemoryContext):
    ...
    await event.message.delete()
    await event.message.answer(text='Успешно!✅ ' + ('Оплачено' if message_data["is_add"] else 'Вычтено') +
